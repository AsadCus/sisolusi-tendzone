...
const ServiceCard = ({
  ...
  <div className="relative bg-white border border-gray-100 shadow-sm p-6 flex flex-col gap-4 hover:shadow-md transition-shadow duration-300">
    <div>
      <h3 className="text-[16px] font-extrabold text-gray-900 mb-1.5">{title}</h3>
      <p className="text-[13px] text-gray-500 leading-relaxed" style={{ textAlign: "justify" }}>
        {description}
      </p>
    </div>
...
const BestSellingTabs = () => {
  ...
  return (
    <div className="overflow-hidden border border-gray-100 shadow-sm">
      <div className="flex overflow-x-auto border-b border-gray-100 bg-gray-50">
        {bestSelling.map((item, i) => (
          <button
            key={item.id}
            onClick={() => setActive(i)}
            className={`shrink-0 px-5 py-3 text-[12px] font-semibold tracking-wide transition-all duration-200 border-b-2 whitespace-nowrap ${
              active === i
      ...
      <div key={active} className="bg-white p-6">
        <div className="flex flex-col md:flex-row gap-6 items-start">
          <div className="shrink-0">
            <div className="w-9 h-9 rounded-xl bg-red-500 flex items-center justify-center shadow-md">
          ...
          <div className="flex-1">
            <h3 className="text-[16px] font-extrabold text-gray-900 mb-2 leading-snug">
              {current.title}
            </h3>
            <p className="text-[13px] text-gray-600 leading-7" style={{ textAlign: "justify" }}>
              {current.description}
            </p>
          </div>
        </div>
    ...
    <main className="w-full min-h-screen bg-white">
      ...
      <section className="w-full bg-white border-b border-gray-100 pt-10 pb-8">
        <div className="max-w-5xl mx-auto px-8 md:px-16 text-center">
          ...
          <p className="text-[13.5px] text-gray-500 leading-7 max-w-2xl mx-auto">
        ...
        <div className="max-w-5xl mx-auto px-8 md:px-16">
          <div className="text-center mb-6">
            ...
            <div className="flex justify-center mt-2">
              <span className="block w-8 h-0.5 bg-red-500 rounded-full" />
            </div>
          </div>
          <BestSellingTabs />
        </div>
